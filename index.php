 ...
 * @Last Modified time: 2018-12-18 00:29:44
    ...
    case 'scan':
        ...
        if ($scan == 1 && !empty($path)) {
            ...
            if (empty($path)) {
                ...
                $dir_list = filterFile($dir_list, $base_dir);
            } else {
                $dir_list = scandir($path);
                $dir_list = filterFile($dir_list, $path);
            }
...
function filterFile($dir_list, $path)
{
    foreach ($dir_list as $key => $temp_dir) {
        if (!is_dir($path.'/'.$temp_dir)) {
            unset($dir_list[$key]);
        }
    }
    return $dir_list;
